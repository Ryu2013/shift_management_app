...
abort("[seed] 本番環境では seeds.rb は実行されません。") if Rails.env.production?
ActiveRecord::Base.transaction do
  ...
  # Employee user
  employee_email = ENV.fetch("SEED_EMPLOYEE_EMAIL", "employee@example.com")
  employee_password = ENV["SEED_EMPLOYEE_PASSWORD"] || SecureRandom.base58(16)

  employee = User.find_or_initialize_by(email: employee_email)
